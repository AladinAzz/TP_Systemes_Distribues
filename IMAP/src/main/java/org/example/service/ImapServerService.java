...
public class ImapServerService implements ServerObserver {
    ...
    public ImapServerService(SimpMessagingTemplate messagingTemplate) {
        ...
        try {
            java.io.File logDir = new java.io.File("IMAP/logs");
            if (!logDir.exists()) {
                logDir.mkdirs();
            }
            this.fileLogger = new PrintWriter(new FileWriter("IMAP/logs/imap_server.log", true), true);
        } catch (IOException e) {
            System.err.println("Impossible de créer le fichier de log IMAP: " + e.getMessage());
        }
    }
