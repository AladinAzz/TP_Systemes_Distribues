...
public class Pop3ServerService implements ServerObserver {
    ...
    public Pop3ServerService(SimpMessagingTemplate messagingTemplate) {
        ...
        try {
            java.io.File logDir = new java.io.File("POP3/logs");
            if (!logDir.exists()) {
                logDir.mkdirs();
            }
            this.fileLogger = new PrintWriter(new FileWriter("POP3/logs/pop3_server.log", true), true);
        } catch (IOException e) {
            System.err.println("Impossible de créer le fichier de log POP3: " + e.getMessage());
        }
    }
